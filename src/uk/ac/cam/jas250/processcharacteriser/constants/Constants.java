/*
 * 
 * A convenient place to put experiment constants. Useful for a 'one-click' experiment.
 * 
 */

package uk.ac.cam.jas250.processcharacteriser.constants;

public final class Constants {
	
	/*
	 * Input file path
	 */
	public static final String INPUT_FILE_PATH = "../my_input_file.txt";
	
	/*
	 * Output file path
	 */
	public static final String OUTPUT_FILE_PATH = "../my_output_file.txt";
	
	/*
	 * Filter for a certain process
	 */
	public static final String PROCESS_FILTER = "";
	
	/*
	 * The size of the window to use
	 */
	public static final int WINDOW_SIZE = 5;
	
	/*
	 * Trace types comment out accordingly 
	 */
	public static final String TRACE_TYPE = "FTRACE";
	//public static final String TRACE_TYPE = "SYSTEMTAP";
}
